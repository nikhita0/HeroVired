...
//for getting the list of products from server
const allProducts = async (req, res) => {
  try{
    ProductModel.find().then((err,response)=>{
    if(err)
        res.send(err);
    else
        res.send({status:200, numberOfentries:response.length, products:response});    
});
  }
  catch (error) {
    // Handle unexpected errors, log them, and provide a generic error message
    console.error("Error during product display:", error);
    return res.status(404).json({
      message: error.message,
    });
  }
...
const searchById = async (req, res) => {
  try{
    ...
    ProductModel.findById().then(idQuery,function(err,response){
          if(err)
              res.send(err);
          else
              res.send({status:200, numberOfentries:response.length, products:response});        
    ...
    // Handle unexpected errors, log them, and provide a generic error message
    console.error("Error while searching a product:", error);
    return res.status(404).json({
      message: error.message,
    });
  }
...
const getBycategory = async (req, res) => {
  try{
    const categoryNameQuery = req.query.category;

    ProductModel.find().then({category:categoryNameQuery},function(err,response){
          if(err)
              res.send(err);
          else
              res.send({status:200, numberOfentries:response.length, products:response});    
    ...
    // Handle unexpected errors, log them, and provide a generic error message
    console.error("Error while searching a product by category:", error);
    return res.status(404).json({
      message: error.message,
    });
  }
